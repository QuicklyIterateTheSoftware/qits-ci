 ...
 * qits:system} and its own project's claim was 403 here while every other {@code /ci/api} read
 * answered it, which made the documented manual re-fire mechanism unusable by exactly the callers
 * that need it.
 *
 * <p>Three things travel with that and none is a widening:
 *
 * <ul>
...
public class CiEventController {
   ...
   *
   * <p>{@code null} is every project. Two callers get it — an operator on the edge's forwarded
   * session, who carries no bearer and has already been judged by this method's roles, and a machine
   * granted {@code project=*}. Anything else is a machine's own project, and the evaluation is
   * narrowed to it.
   *
   * <p>The order of the questions is the contract, and each one has a status code behind it: no
   * machine token at all is not a machine (the forwarded arm, or a 401 from {@link
   * MachineAuth#require()} when the gate is on and nothing was presented); a token addressed
   * elsewhere is a 403 from the same call; a token carrying no {@code project} claim is a 403 here,
   * because an absent claim is a mismatch and never a wildcard. What this method never does is let a
   * caller widen its own check: {@code "*"} is read on the token side only, exactly as {@code
   * MachineIdentity} reads it.
   */
  private String scopeOf() {
    if (!MachineIdentity.isMachine(identity)) {
      return null;
    }
    // Presence and audience only — the claim is this method's question, not the library's, because
    // "granted some project" is not an equality check.
    machineAuth.require();
    String project =
        MachineIdentity.claim(identity, QitsClaims.PROJECT)
            .orElseThrow(
                () -> new ForbiddenException("Token " + QitsClaims.PROJECT + " claim is required"));
